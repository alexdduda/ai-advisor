...
// API client for clubs endpoints — falls back to static McGill club data if backend unavailable
...
// ── Static McGill clubs data (used when DB is empty or unavailable) ──────────
const STATIC_CLUBS = [
  ...
  { id: 's17', name: 'McGill Mathematics & Statistics Society',category: 'Academic',                description: 'Hosting mathematical seminars, study sessions, and the annual Putnam competition prep workshops for math students.',                 member_count: 175, is_verified: true, website_url: null,                              meeting_schedule: 'Tuesdays 5 PM — Burnside Hall', contact_email: 'mathstats@ssmu.ca' },
...
const clubsAPI = {
  ...
  async getStarterClubs(userId, major) {
    ...
    // Fallback: pick clubs relevant to major from static list
    const majorLower = (major || '').toLowerCase()
    const relevant = STATIC_CLUBS.filter(c =>
      (majorLower.includes('computer') && c.category === 'Engineering & Technology') ||
      (majorLower.includes('engineer') && c.category === 'Engineering & Technology') ||
      (majorLower.includes('biology') && (c.category === 'Science' || c.name.includes('Bio'))) ||
      (majorLower.includes('management') && c.category === 'Professional') ||
      (majorLower.includes('law') && c.name.includes('Law')) ||
      (majorLower.includes('music') && c.name.includes('Music')) ||
      c.name.includes('McGill AI') || c.name.includes('Debate') || c.name.includes('Outdoors')
    ).slice(0, 5)
    const fallback = relevant.length ? relevant : STATIC_CLUBS.slice(0, 5)
  ...
  async joinClub(userId, clubId) {
    try {
      ...
      // If it's a network error (not a 409), treat as soft success so UI updates
  ...
  // Expose static data for components that need it directly
  STATIC_CLUBS,
}
