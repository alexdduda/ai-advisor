...
export function generateInsights(userData) {
  ...
  if (profile?.interests) {
    ...
    insights.push({
      ...
      content: `Explore courses in ${interests.slice(0, 3).join(', ')}`
    })
  }
  ...
  if (profile?.major) {
    insights.push({
      type: 'peer',
      icon: '👥',
      title: 'Students in your major also took',
  ...
  if (chatTopics.length > 0) {
    insights.push({
      type: 'chat',
      icon: '💬',
      title: 'You frequently ask about',
  ...
  if (profile?.current_gpa >= 3.7) {
    insights.push({
      ...
      content: "Your GPA qualifies you for honours programs and competitive opportunities"
    })
  }
  ...
  if (remaining > 0 && remaining <= 30) {
    insights.push({
      ...
      icon: '🎓',
      title: 'Almost there!',
      content: `Only ${remaining} credits until graduation`
    })
  }
