...
from .clubs_email import (
    _send_join_request_email,
    _notify_club_members_new_event,
    _notify_club_members_announcement,
    _generate_action_tokens,
...
# ════════════════════════════════════════════════════════════════════
#  Stale-club cleanup
#  Delete a club if neither the owner nor any admin has signed in to
#  Symbolos for over STALE_CLUB_THRESHOLD_DAYS. Runs daily from the
...
def run_stale_club_cleanup_cron(dry_run: bool = False) -> dict:
    """
    For each verified club: collect every owner + admin user_id, look up
    their last sign-in via Supabase auth, and if the most recent sign-in
    across the whole admin team is more than STALE_CLUB_THRESHOLD_DAYS old,
    delete the club.
    ...
    try:
        ...
        for club in all_clubs:
            ...
            # Collect every admin/owner user_id for this club
